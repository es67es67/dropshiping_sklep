...
async function updateUsersWithTeryt() {
  try {
    ...
    // Pobierz wszystkich użytkowników z adresami
    ...
    for (const user of users) {
      ...
      // Sprawdź czy użytkownik ma już kody TERYT
      if (user.teryt && user.teryt.fullCode) {
        console.log(`⚠️ Użytkownik ${user.username} już ma kody TERYT: ${user.teryt.fullCode}`);
        skippedCount++;
        continue;
      }
      ...
      if (user.address && user.address.city) {
        ...
        if (cityInfo) {
          ...
          if (voivodeshipEntry) {
            ...
            user.teryt.voivodeshipCode = voivodeshipCode;
            user.teryt.simcCode = simcCode;
            
            // Generuj przykładowe kody powiatu i gminy (w rzeczywistości pobierałoby się z bazy GUS)
            user.teryt.countyCode = voivodeshipCode + '01'; // Przykładowy kod powiatu
            user.teryt.municipalityCode = voivodeshipCode + '0101'; // Przykładowy kod gminy
            user.teryt.tercCode = voivodeshipCode + '0101'; // Pełny kod TERC
            
            // Dodaj kod ulicy jeśli jest dostępny
            if (user.address.street) {
              const streetName = user.address.street.replace(/^ul\.\s*/i, '').trim();
              const streetCode = streetUlicCodes[streetName];
              if (streetCode) {
                user.teryt.ulicCode = streetCode;
              }
            }
      ...
      if (updated) {
        ...
        if (user.teryt.tercCode && user.teryt.simcCode && user.teryt.ulicCode) {
          user.teryt.fullCode = `${user.teryt.tercCode}${user.teryt.simcCode}${user.teryt.ulicCode}`;
        } else if (user.teryt.tercCode && user.teryt.simcCode) {
          user.teryt.fullCode = `${user.teryt.tercCode}${user.teryt.simcCode}`;
        } else if (user.teryt.tercCode) {
          user.teryt.fullCode = user.teryt.tercCode;
        }
        
        await user.save();
        console.log(`✅ Zaktualizowano użytkownika: ${user.username} - ${user.teryt.fullCode}`);
        updatedCount++;
      } else {
        console.log(`⚠️ Nie udało się znaleźć kodów TERYT dla użytkownika: ${user.username}`);
        skippedCount++;
      }
    }

    console.log(`\n📊 Podsumowanie aktualizacji użytkowników:`);
    console.log(`   ✅ Zaktualizowano: ${updatedCount} użytkowników`);
    console.log(`   ⚠️ Pominięto: ${skippedCount} użytkowników`);
