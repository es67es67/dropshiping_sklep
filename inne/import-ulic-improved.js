...
// Statystyki
let stats = {
  ULIC: { processed: 0, imported: 0, updated: 0, errors: 0, skipped: 0 }
};
...
// Funkcja do sprawdzania czy rekord istnieje - sprawdza WSZYSTKIE pola kluczowe
async function checkRecordExists(record, type) {
  try {
    // Dla ULIC sprawdzamy wszystkie pola kluczowe
    const keyFields = {
      WOJ: record.WOJ,
      POW: record.POW,
      GMI: record.GMI,
      RODZ_GMI: record.RODZ_GMI,
      SYM: record.SYM,
      SYM_UL: record.SYM_UL,
      CECHA: record.CECHA,
      NAZWA_1: record.NAZWA_1,
      NAZWA_2: record.NAZWA_2,
      STAN_NA: record.STAN_NA
    };

    const response = await axios.post(`${BACKEND_URL}/api/export/check-record-complete`, {
      record: keyFields,
      type: type
    });
...
// Funkcja do importu danych z poprawioną logiką duplikatów
      ...
      try {
        ...
        for (const record of batch) {
          ...
          if (checkResult.exists && checkResult.isComplete) {
            // Tylko pomijaj jeśli WSZYSTKIE pola kluczowe są identyczne
            log(`⏭️ Pomijam istniejący rekord (wszystkie pola identyczne): ${record.NAZWA_1} ${record.NAZWA_2}`, 'info');
            ...
            log(`🔄 Aktualizuję niekompletny rekord: ${record.NAZWA_1} ${record.NAZWA_2}`, 'warning');
          } else {
            // Rekord nie istnieje lub istnieje tylko częściowo - dodaj go
            processedBatch.push({
              ...record,
              action: 'insert'
            });
            log(`➕ Dodaję nowy rekord: ${record.NAZWA_1} ${record.NAZWA_2}`, 'info');
          }
        }
...
// Funkcja główna - tylko ULIC z poprawioną logiką
async function main() {
  try {
    log('🚀 Rozpoczynam import DANYCH ULIC GUS do bazy danych (POPRAWIONA LOGIKA DUPLIKATÓW)');
    ...
    log('📊 PODSUMOWANIE IMPORTU ULIC:', 'info');
    log(`ULIC: ${finalStatus.data.ulic} rekordów`, 'info');
    log(`Przetworzono: ${stats.ULIC.processed}`, 'info');
    log(`Zaimportowano: ${stats.ULIC.imported}`, 'info');
    log(`Zaktualizowano: ${stats.ULIC.updated}`, 'info');
    log(`Pominięto (duplikaty): ${stats.ULIC.skipped}`, 'info');
    log(`Błędy: ${stats.ULIC.errors}`, 'info');
    
    log('✅ Import danych ULIC zakończony pomyślnie!', 'success');
